import {Theme} from "./Theme";  
  export default function generateDarkTheme(hexCode: string): Theme {
    ...
    const inverseSurface = lightenRgb(primary, 0.6); // Lighten the primary color for inverse surface
    const inverseOnSurface = darkenRgb(primary, 0.6); // Darken the primary color for contrast on inverse surface
    const inversePrimary = darkenRgb(primary, 0.2);
    const elevation = {
      level0: "transparent",
      level1: darkenRgb(primary, 0.92),
      level2: darkenRgb(primary, 0.88),
      level3: darkenRgb(primary, 0.84),
      level4: darkenRgb(primary, 0.80),
      level5: darkenRgb(primary, 0.76)
    };
